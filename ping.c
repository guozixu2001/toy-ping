...
int icmp_unpack(char *buf, int len) {
  ...
  if ((icmp->icmp_type == ICMP_ECHOREPLY) &&
    ...
    printf("%d byte from %s: icmp_seq=%u ttl=%d rtt=%d ms\n", len,
           inet_ntoa(ip_hdr->ip_src), icmp->icmp_seq, ip_hdr->ip_ttl, rtt);
...
void ping_send() {
  ...
  while (alive) {
    ...
    size = sendto(rawsock, send_buf, 64, 0, (struct sockaddr *)&dest,
                  sizeof(dest));
...
int main(int argc, char *argv[]) {
  ...
  setsockopt(rawsock, SOL_SOCKET, SO_RCVBUF, &size,
             sizeof(size)); //增大接收缓冲区至128K
